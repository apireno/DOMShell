...
### Hardening — MCP server (`2.0.9`)

These close the design gaps that let a stale native server squat undetected for days alongside the intended container (kgspin 2026-07 RCA — the "two servers on one machine" collision, root-caused to native-server-spawn instructions in integrator standup scripts + a host-scoped singleton guard + an identity-blind proxy).

- **Interface-agnostic singleton guard** ([`mcp-server/index.ts`](https://github.com/apireno/DOMShell/blob/main/mcp-server/index.ts)). The existing `EADDRINUSE` exit only catches a *same-host-scope* collision, so a native server on `127.0.0.1:9876` and a container on `0.0.0.0:9876` (bound inside its own netns, mapped by Docker as IPv6 `*:9876`) coexisted silently — the extension attached to one, the other became an invisible squatter that proxies relayed to. On startup the server now probes **both** `127.0.0.1` and `::1` on its WS port with a deliberately-invalid token; a DOMShell bridge answers by completing the upgrade then closing with code `4001`, so any existing bridge is detected across the IPv4/IPv6 + host-mapping boundary. If one is found the server **refuses to start** with an explicit message (`--allow-duplicate` overrides for a deliberate multi-bridge rig). The guard is asymmetric by nature: a native server *can* see a running container (host port-mapping is reachable from the host netns) — the common "container running, then an agent spawns a native" failure mode — but a container cannot see a host-native from inside its netns.
- **Proxy logs the server identity it reached.** `domshell-proxy` previously relayed blindly to whatever squatted its `--port`. It now does a one-shot `initialize` on startup and logs `relaying to DOMShell v<version> at <origin>` to stderr, so a version/endpoint mismatch is visible instead of silent. Best-effort; a probe failure never blocks the relay.
- **`initialize` serverInfo now reports the real version.** It was a stale hardcoded `"1.0.0"`, useless for identifying which DOMShell instance a client reached; it now reports `MCP_SERVER_VERSION`, which is what makes the proxy identity log (and any client-side version assertion) meaningful.
