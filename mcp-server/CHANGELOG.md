...
- **`key` command** — dispatch a single trusted keyDown+keyUp pair (CDP `Input.dispatchKeyEvent`) to the currently-focused element. Pairs naturally with `focus <name>` for SPAs whose activation handler listens for Enter, Escape, or arrow keys rather than mouse clicks. Built-in names: `Enter`, `Escape`, `Tab`, `Backspace`, `Delete`, `Space`, `ArrowUp/Down/Left/Right`, `Home`, `End`, `PageUp`, `PageDown`, `F1`–`F12`, plus single characters with `--modifiers ctrl,shift,alt,meta`. Two non-obvious things had to be right for the keystroke to actually fire: (1) special keys (Escape, arrows, F-keys) go through CDP's `rawKeyDown` path — `keyDown` without a `text` field is silently dropped — while text-producing keys (Enter='\r', Tab='\t', Space=' ') and printable chars use `keyDown` with `text` set; (2) the dispatch issues `Page.bringToFront` first because `chrome.debugger`-routed `Input.dispatchKeyEvent` is silently no-op'd by Chrome when the target tab isn't the active tab in a focused window (mouse events are exempt because they carry coordinates and Chrome hit-tests by position). Side effect: invoking `key` visibly brings the tab to front; consecutive keys to the same tab cost the shift only once. Write tier. Motivated by LinkedIn's conversation list, whose `<div tabindex="0">` rows activate on Enter — see #37. (#40)
